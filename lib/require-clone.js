...
function requireClone(providedPath, parentPath = module.parent.filename) {
	return __requireClone(providedPath, parentPath);
}

function __requireClone(providedPath, parentPath) {
    let modulePath = getPath(providedPath, parentPath);
    let requiredModule = new Module(modulePath);
    try {
        requiredModule.require = R.curry(__requireClone)(R.__, requiredModule.id);
